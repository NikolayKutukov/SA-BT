"""Loss functions for SurvTRACE (vendored + cleaned up)."""

import torch
from torch import Tensor
import torch.nn.functional as F

from .utils import pad_col, log_softplus
...
class NLLLogistiHazardLoss(_Loss):
    """Negative log-likelihood of the logistic hazard parametrisation."""

    def forward(
        self, phi: Tensor, idx_durations: Tensor, events: Tensor, reduction: str = "mean"
    ) -> Tensor:
...
class NLLPCHazardLoss(_Loss):
    """Negative log-likelihood of the piecewise-constant hazard model."""
